...
import com.example.familytree.databinding.ActivityFamilyActivityDetailBinding
        ...
        viewModel.activity.observe(this) { activity ->
            if (activity != null) {
                ...
                binding.textType.text = when (activity.activityType) {
                    com.example.familytree.data.model.ActivityType.WORSHIP -> "祭祖活动"
                    com.example.familytree.data.model.ActivityType.REUNION -> "宗亲聚会"
                    com.example.familytree.data.model.ActivityType.CULTURAL -> "文化活动"
                    else -> "其他"
                }
                
                // 设置活动状态
                val statusText = when (activity.status) {
                    com.example.familytree.data.model.ActivityStatus.UPCOMING -> "即将开始"
                    com.example.familytree.data.model.ActivityStatus.ONGOING -> "正在进行"
                    com.example.familytree.data.model.ActivityStatus.COMPLETED -> "已结束"
                }
